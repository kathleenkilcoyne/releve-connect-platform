...
| 17 | Team join invite | A Team Director clicks **Send** on the "Invite your {members} by email" box on `/studio/schedule` (dance teams only). One email per address in the list; requires the team's join code to already exist (never minted from here). Never states or implies the Roster or The Swing. | Each invited address | `team-join-invite.v1` | ✅ implemented |
