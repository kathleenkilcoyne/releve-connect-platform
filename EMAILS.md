...
| 17 | Founding Professional invitation | **Admin grants (or resends) Founding Professional status** in `/admin/founding-professionals`. Carries the code-generated `foundingProfessionalInviteLink()` — never a hand-composed URL. Gated on `FOUNDING_PROFESSIONAL_AUTOSEND_ENABLED` (default OFF); every attempt (sent/failed/not_configured) is logged in `founding_professional_invitation_sends`, whether or not the flag is on. | Invited professional | `founding-professional-invitation.v1` | ✅ implemented, **autosend OFF by default (2026-09-07)** |
