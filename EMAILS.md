...
| 14 | Your studio/team is live | **Admin publishes** an org (`approved` → `live`) in `/admin/studios`. Wording branches on `org_type` via `orgCopy()` (fix, 2026-09-01 — previously said "studio page" unconditionally, including to Dance Teams) — "studio page" for `org_type = studio`, "team page" for `org_type = dance_team`. Same template id either way, not a separate `.v1`/`v2` (copy-only branch, same shape as #12/#12b). | Studio owner / Team Director | `studio-live.v1` | ✅ implemented |
...
| 17 | Booking confirmed (buyer) | Professional Services transaction rail, Phase 1 (2026-09-01). A service/session booking's Stripe Checkout Session completes (`checkout.session.completed`, `kind: service_booking`). Shows the buyer's total charge broken out as price + the 3% Relevé booking fee — never folded silently into the Professional's price. | Buyer | `service-booking-confirmed-buyer.v1` | ✅ implemented |
| 18 | New booking (Professional) | Same trigger as #17, same webhook event. Shows the Professional their own payout (their full stated price — Stripe's processing fee is deducted at settlement, not by this email or by Relevé). | Professional | `service-booking-confirmed-professional.v1` | ✅ implemented |
