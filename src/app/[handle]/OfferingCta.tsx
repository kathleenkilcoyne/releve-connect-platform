...
// column.
//
// The Inquire interaction itself (2026-08-18) lives in InquireButton.tsx,
// shared with AvailabilityWindowsSection — this file only computes the prefill
// text and label an Offering needs; the send/error/pending/sent machinery is
// identical either way, by construction.

import Link from "next/link";
import { introPrefillMessage, type ResolvedCta } from "@/lib/offerings";
import { CTA_BTN, InquireButton } from "./InquireButton";
...
export default function OfferingCta({
  ...
  if (cta.action === "external") {
    return (
      <a href={cta.href} target="_blank" rel="noopener noreferrer" className={CTA_BTN}>
  ...
  if (cta.action === "licensing") {
    return (
      <Link href={cta.href} className={CTA_BTN}>
        {cta.label} <span aria-hidden>→</span>
      </Link>
    );
  }
