...
describe("saveProfile payload — members can never write a Relevé trust signal", () => {
  // A source-level guard. These five fields are conferred by Relevé, never by a
  // member's form save, and the profile editor's action must not reference them
  // in any executable line. Comments are stripped first, because the file
  // deliberately NAMES them in a warning block.
  it("does not reference any trust-signal column outside comments", () => {
