...
- Status & standing: `status` (pending / in-review / approved) · `profile_status` (draft / published) · `visibility` (public / unlisted) · `verification_flag` (**Verified Member** — identity/standing mark, granted at profile creation once vetting is complete (approved + paid), no waiting period; not a competence stamp) · `choreographer_tier` (defaults to Emerging; **admin-assigned / founder-reviewed** earned ladder — no hardcoded threshold) · `founder_distinction` · `updated_at`.
...
4. RESOLVED (updated 2026-07-12): the **Verified Member** mark (renamed from "Certified") is RC-granted **immediately at profile creation once vetting is complete — approved (documentation-authenticity check passed) AND paid — with no waiting period** (supersedes the earlier "~60 days from activation"). It is an **identity / standing mark** (real, vetted, active member), **not** a competence stamp — RC never vouches for skill (no-endorsement).
