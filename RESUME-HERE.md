...
> ## ▶️ END-OF-NIGHT CHECKPOINT (2026-08-21, ~04:40 ET) — hard stop, fully backed up, explicit resume point
>
> **Branch `profile/profile-ux-walkthrough`. Four commits landed tonight since `0fc7ab9` (Slice D), all pushed to `origin/profile/profile-ux-walkthrough`. `main` untouched (`79bbfdb`, unmoved all night — 1 docs-only commit ahead of the shared fork point). No PR, no merge, no deploy. The sibling branch (`feature/this-week-ui-redesign`) was not touched, merged, or cherry-picked beyond the deliberate, narrow reconciliation already described below.**
>
> Commit order tonight: `45097bd` (Roster rebuild + public-profile reconciliation Steps 1–2 + What I Offer discoverability) → `fa9425f` (generic Availability removed from Roster UI) → `cd7dc3d` (docs checkpoint) → `0b4a253` (migration-history reconciliation: 35 renames + 10 reconciled files + 1 test-path fix) → `0288b02` (Step 2: stale `primary_role` fixed on `[handle]/page.tsx` + `roster/saved/page.tsx`).
>
> ### Where each piece of tonight's work stands
> 1. **Migration-history reconciliation — DONE.** A read-only audit found this branch's local migration filenames had drifted from the live Supabase ledger's real version numbers — not just the 10 migrations brought over from the sibling branch, but ~35 older migrations native to this branch's own history. All 35 renamed via `git mv` (zero content diff, verified via `git diff --stat -M100%` showing `0 insertions/deletions` on every one) to match their real ledger versions. **Still open, deliberately not done tonight:** 8 migrations applied live with no local file on this branch at all (`application_draft_fields`, `generalize_activations`, `music_bucket_and_current_track`, `professional_activations`, `profile_views`, `studio_classes_kind`, `substitution_engagements_must_be_class_scoped`, `works_and_licensing`) — already safely reconciled once before on a sibling branch (2026-08-17, commit `508b36f`), just never brought onto this one; and 4 local-only files predating the ledger entirely (`enable_row_level_security`, `stripe_connect_signature_experience`, `add_consumer_account_type`, `vetting_gate_application_fee`) — almost certainly initial baseline setup, left alone.
> 2. **Generic Availability — REMOVED from the Roster filter UI.** The "Availability" / "Currently accepting" chip filters and their `availability_tags` fetch are gone from `/roster` (founder decision: Available This Week is the real, actionable answer to "when can I book this person," not vague chips). `availability_tags`, `profile_availability`, and the view's `availability_slugs` column are all untouched — UI-only removal, fully reversible. The profile editor already had zero generic Availability controls (Slice C, predates tonight) — verified, not re-implemented.
> 3. **Public-profile reconciliation (Steps 1–2 of the narrow plan) — DONE**, not a full merge of the sibling branch. Landed 4 new files verbatim (`AvailabilityWindowsSection.tsx`, `InquireButton.tsx`, `SelectedWorkSection.tsx`, `public-availability.ts`+test). Rebuilt `[handle]/page.tsx`'s hierarchy: **Header → Bio → My Services → Available This Week → Selected Work → Credentials → Links.** Standalone Styles/Levels/Focus/Availability tag rows removed from the public render (data/editor/Roster search untouched — commented out, not deleted). "Touring with" removed from public rendering entirely (field/data untouched); "Teaching at" kept as one understated header line. **Still open:** `OfferingCta.tsx` still has its own unrefactored local `InquireButton` — never swapped to use the shared component landed tonight (this is CTA reconciliation, explicitly deferred — see the resume order below).
> 4. **What I Offer discoverability + naming — DONE.** Root-caused via a live data audit: Geoffrey Doig-Marx (Founding Professional) has zero `professional_offerings` rows because the feature was flag-gated off in production when he built his profile — confirmed `PROFESSIONAL_OFFERINGS_ENABLED` is now live in production, so this is a going-forward fix, not a flag flip. Post-save onboarding CTA ("Next: Add What You Offer →") now shows whenever a member has zero offerings, persists until they add one (Option B), and disappears once they have ≥1. `/profile` dashboard tile and `/profile/offerings` H1 both renamed "Professional Offerings"/"My Offerings" → **"What I Offer"**. Verified against Geoffrey's real zero-offerings state via automated test, not by signing in as him. **Geoffrey has not been sent any outreach yet** — still to be drafted/sent, separately from code work.
> 5. **Merge-readiness Step 2 — DONE tonight.** The stale `primary_role` display (flagged in the reconciliation plan) is fixed on `[handle]/page.tsx` (added a `profile_roles` fetch alongside the existing styles/levels/focus/availability joins, reusing the file's own `labelsOf` helper) and on `roster/saved/page.tsx` (added a second batched `profile_roles → role_types` query, since this page deliberately reads `talent_profiles` directly rather than the `roster_profiles` view, to keep showing unlisted-but-saved profiles). Both now display roles the same way the Roster cards already did (fixed earlier tonight).
>
> ### Verification status right now
> - **Public profile (`/kathleen-mcaree`), visually verified with real data:** header now reads "Teacher / Educator" (the real `role_types` label) instead of the old titleCased raw slug. Zero new console errors (one pre-existing 404 appears identically on the untouched homepage — unrelated).
> - **`/roster/saved`:** loads cleanly with its empty state (this account has no saved profiles right now) — proves the added query doesn't error, but **a populated card's role display was deliberately NOT visually tested**, since that would have required saving/unsaving a real profile as a live test action. Explicitly not done, per instruction, to avoid mutating real data this late at night.
> - **`tsc --noEmit`:** clean.
> - **Full test suite:** 400 passed, 2 pre-existing skips — unchanged all night.
> - **ESLint:** one pre-existing accepted warning (`TagRow` in `[handle]/page.tsx`, a deliberately-kept dormant helper matching the sibling branch's own precedent) + a handful of pre-existing errors/warnings in files untouched tonight (confirmed via `git log`) — nothing new introduced by any of tonight's work.
> - **Supabase security advisors:** no new findings from anything applied tonight.
>
> ### Exact remaining merge-readiness items, in the agreed order
> 1. **`VALID_ROLES` in `apply/actions.ts`** — still hardcodes only the original 4 roles; new applicants can't select any role added since 2026-08-19 (including the 9 added tonight) at intake. Next task to resume with.
> 2. **CTA reconciliation** — swap `OfferingCta.tsx` to use the shared `InquireButton.tsx` landed tonight, removing its duplicate local implementation.
> 3. **The deliberate decision on remaining sibling-branch pieces** — the Roster "My Services" facet (needs reimplementing on tonight's new filter architecture, not porting), the Professional Services (other-businesses) directory, the `/this-week` internal-dashboard redesign, and the 8 still-missing migration files above. Not a merge blocker by itself; a scoping conversation.
> 4. **Then, and only then, open the PR** to `main`.
>
> Do not wander back into profile design or start any of the above out of order — this is the exact resume point.

> ## ✅ ROSTER REBUILD + PUBLIC PROFILE RECONCILIATION + WHAT I OFFER DISCOVERABILITY — two checkpoints DONE, pushed, NOT merged (2026-08-21, night)
>
> **Branch `profile/profile-ux-walkthrough`, two commits tonight, both pushed to `origin/profile/profile-ux-walkthrough`. `main` untouched — nothing PR'd, nothing merged, nothing deployed.** Starting point was `0fc7ab9` (Slice D). Commit order: `45097bd` → `fa9425f` (current tip).
>
> ### Commit `45097bd` — Roster search rebuild + public profile reconciliation + What I Offer discoverability
> - **Roster Phase 1 (founder-approved):** Professional Role is now a real multi-select facet on `role_slugs` (via `profile_roles`), not the deprecated single `primary_role` column — the role list is read live from `role_types` (new `show_in_roster` column), not a hardcoded array. 9 new roles added (Vocal/Acting/Dance/Audition Coach, Music Director / Accompanist, Stage Manager, Casting Director, Dance Captain / Assistant Choreographer, **College / University Faculty** — deliberately a ROLE, not a separate teaching-context facet). New **Professional Experience** taxonomy (Broadway, Off-Broadway, National Tour, International Tour, Regional Theatre, Film/TV, Commercial, Concert/Cabaret, Professional Company) with its own Roster filter + editor section. **Focus Areas** exposed to Roster filtering for the first time. Roster cards display roles from `role_slugs`, not stale `primary_role`. Migrations: `add_professional_roles_phase1`, `professional_experience_taxonomy`, `roster_view_role_focus_experience`.
> - **Same-night regression, found and repaired:** the Phase 1 view rebuild unintentionally dropped `service_slugs` — a column a *sibling, unmerged branch* (`feature/this-week-ui-redesign`) had added directly to the shared **live** `roster_profiles` view (never committed to this branch's local migration history). Restored via migration `restore_service_slugs_on_roster_view`. No offering data was ever at risk — only the view's exposure of it was briefly lost. **Lesson recorded:** this repo has only ONE Supabase project; every branch's `npm run dev` hits the same live database, so a DB-side change on one branch can silently break another's assumptions. Worth reconciling the ~10 migration files that exist live but aren't in this branch's `supabase/migrations/` folder (see the reconciliation plan below).
> - **Public profile reconciliation (Steps 1–2 of a deliberately narrow plan — NOT a wholesale merge):** landed 4 new, previously-unowned files verbatim from `feature/this-week-ui-redesign` (`AvailabilityWindowsSection.tsx`, `InquireButton.tsx`, `SelectedWorkSection.tsx`, `public-availability.ts`+test). Rebuilt `[handle]/page.tsx`'s hierarchy: **Header → Bio → My Services → Available This Week → Selected Work → Credentials → Links.** Standalone Styles/Teaching Levels/Focus/Availability tag rows removed from the *public* render (data/editor/Roster search untouched — commented out, not deleted). Featured Reel moved out of the hero into Selected Work (no longer assumed to be a "Teaching Reel"). This branch's own portrait-headshot fix was preserved as-is; `primary_role` display and multi-role architecture deliberately left untouched (known gap, see below). **"Touring with" removed from public rendering entirely** (field/data untouched); **"Teaching at" kept as one understated line** in the header, no separate "Currently" section.
> - **What I Offer discoverability + naming fix**, root-caused via a live data audit: **Geoffrey Doig-Marx** (Founding Professional, profile built 2026-08-19) has zero `professional_offerings` rows because the feature was flag-gated OFF in production at the time — confirmed via history AND the fact that `professional_offerings` had exactly 6 rows total in the whole database, all under Kathleen's own profile, before this fix. **`PROFESSIONAL_OFFERINGS_ENABLED` is now confirmed LIVE in production** (Kathleen verified), so this is a *going-forward* discoverability fix, not a flag flip. After every successful profile save, a member with zero offerings now sees **"Next: Add What You Offer →"** (linking to `/profile/offerings`), more prominent than "View my public page"; the nudge disappears once ≥1 offering exists (Option B — persistent-until-started, not one-shot). `/profile` dashboard tile renamed **"Professional Offerings" → "What I Offer"** (subcopy: "Add what you offer" / "Manage what you offer"); `/profile/offerings` H1 renamed **"My Offerings" → "What I Offer"**. Internal code/table/route names (`professional_offerings`, `/profile/offerings`) deliberately unchanged. The two decisions extracted into pure, tested functions (`offeringsTileSubcopy`, `shouldShowOnboardingOfferingsCta`) and verified against **Geoffrey's real zero-offerings state** via automated test — never signed in as him, never touched his data.
>
> ### Commit `fa9425f` — Remove generic Availability filter UI from the Roster
> - Founder decision: a profile shouldn't have to announce vague availability ("Weekends", "Willing to Travel") — **Available This Week** (the real, dated, service-linked booking system landed in the commit above) is the actual answer to "when can I book this person." Removed the "Availability" / "Currently accepting" `FilterChips` blocks and their `availability_tags` fetch from `/roster`.
> - **Left completely untouched, for reversibility:** the `availability_tags` table, `profile_availability` join, the view's `availability_slugs` column, and `RosterFilters.availability` in `filters.ts` — a crafted `?avail=` URL would still filter; only the UI surface is gone. The profile editor already had zero generic Availability controls (Slice C, `008e2a3`, predates tonight).
> - **Available This Week / `service_availability` logic and Swing/This Week scheduling are entirely untouched** by either commit tonight.
>
> ### Verified at every checkpoint tonight
> `tsc --noEmit` clean · ESLint clean (one accepted warning — the dormant `TagRow` helper, matching the source branch's own precedent) · 400 tests passing (2 pre-existing skips) · no new Supabase security-advisor findings · live browser checks against real data (Kathleen's own profile throughout; Geoffrey Doig-Marx's public profile as a second, differently-shaped real profile with zero offerings) at each step, not just at the end.
>
> ### What is genuinely NOT done yet — don't assume otherwise
> - **`feature/this-week-ui-redesign` is NOT merged or cherry-picked.** Only the 4 files + the hierarchy concept above were manually reconciled. That branch still holds, unreconciled: a Roster "My Services" facet (`?svc=`, built on the OLD pre-Phase-1 Roster architecture — needs reimplementing on top of tonight's `filters.ts`, not porting), the "Professional Services" (other-businesses) directory, and a full `/this-week` internal-dashboard redesign.
> - **The ~10 migration files already live on the shared DB but missing from this branch's local `supabase/migrations/`** (professional_services, service_availability_and_bookings, profile_v2_activation, currently_accepting_to_my_services, roster_reads_my_services, merge_private_audition_coaching, availability_points_at_my_services, etc.) have not been copied over for documentation parity.
> - **Known display/architecture gaps, flagged but intentionally not fixed:** Professional Role multi-select is OR-within-facet (selecting two roles is a union, not an intersection — matters for "Audition Coach + College/University Faculty" specifically); `[handle]/page.tsx` and `roster/saved/page.tsx` still display the stale `primary_role` instead of `role_slugs`; `apply/actions.ts`'s `VALID_ROLES` still hardcodes only the original 4 roles, so new applicants can't select any role added since 2026-08-19 (including tonight's 9) at intake.
> - **Geoffrey Doig-Marx has not been sent any outreach yet** about What I Offer now existing — that message is still to be drafted/sent, and the "should a first-time invitee also get a nudge via the welcome email, not just the post-save CTA" question is still open.
> - Phase 2 of the original Roster plan (expanded keyword search + relevance ranking) was never approved and never started.
