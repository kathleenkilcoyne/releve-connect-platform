...
Built to build spec §6 + the §17 guardrails. Three decisions were confirmed with Kathleen up front: Verified Member follows the **§13 60-day earned** rule (not stamped at creation); the public URL is **root `/[handle]`** with a reserved-word guard; and the hero's **earned proof is hidden until real Swing/review data exists**.
...
- **Approval decision transfer:** on **first** profile creation, `saveProfile` copies `applications.honorifics` + `approved_tier` (→ `choreographer_tier`) onto the profile and sets `certified_eligible_at = +60 days` (Verified Member eligibility clock; `verification_flag` stays false — earned, not stamped). Honorifics/verification are **server-stamped only, never form-editable**.
